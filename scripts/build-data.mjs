...
function buildSeasonNotableTankGames(bottomTeams, games, now) {
  const bottomTen = bottomTeams.slice(0, 10);
  const bottomTenIds = new Set(bottomTen.map((team) => team.teamId));
  ...
  for (const game of games) {
    ...
    if (!bottomTenIds.has(homeTeamId) || !bottomTenIds.has(awayTeamId)) continue;
